...
Module publicfunctions
    ...
    Public Sub ReceiptHeader(sender As Object, e As PrintPageEventArgs, VoidReturn As Boolean)
        Try
            ...
            SimpleTextDisplay(sender, e, "Name:", font, 0, 70 + AddLine)
            If SENIORDETAILSBOOL = True Then
                SimpleTextDisplay(sender, e, SeniorDetailsName, font, 30, 65 + AddLine)
            End If

            e.Graphics.DrawLine(Pens.Black, 40, 97 + AddLine, 180, 97 + AddLine)
            SimpleTextDisplay(sender, e, "Tin:", font, 0, 80 + AddLine)
            e.Graphics.DrawLine(Pens.Black, 28, 107 + AddLine, 180, 107 + AddLine)
            SimpleTextDisplay(sender, e, "Address:", font, 0, 90 + AddLine)
            e.Graphics.DrawLine(Pens.Black, 49, 117 + AddLine, 180, 117 + AddLine)
            SimpleTextDisplay(sender, e, "Business Style:", font, 0, 100 + AddLine)
            e.Graphics.DrawLine(Pens.Black, 75, 127 + AddLine, 180, 127 + AddLine)
        Catch ex As Exception
            MsgBox(ex.ToString)
            SendErrorReport(ex.ToString)
        End Try
    ...
    Public Sub ReceiptFooter(sender As Object, e As PrintPageEventArgs, a As Integer, ItemReturn As Boolean)
        Try
            ...
            If ItemReturn Then
                CenterTextDisplay(sender, e, "THIS DOCUMENT SHALL BE ", font1, a + 320)
                CenterTextDisplay(sender, e, "VALID FOR FIVE(5) YEARS FROM THE DATE ", font1, a + 330)
                CenterTextDisplay(sender, e, "OF THE PERMIT TO USE", font1, a + 340)

                CenterTextDisplay(sender, e, "THIS DOCUMENT IS NOT ", font1, a + 350)
                CenterTextDisplay(sender, e, "VALID FOR CLAIM OF INPUT TAX", font1, a + 360)
            End If
        Catch ex As Exception
            MsgBox(ex.ToString)
            SendErrorReport(ex.ToString)
